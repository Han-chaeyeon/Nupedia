...
const MovieSection = ({ title, query, onMovieSelect }) => {
    ...
    const fetchSectionMovies = async () => {
      ...
      try {
        ...
        const { movies: fetchedMovies } =
          await movieService.searchMovieCollection(query, 1);
        setMovies(fetchedMovies);
      } catch (error) {
        console.log(
          `[Service Error] ${title} 섹션 로딩 실패: `,
          error.message || error
        );
        setMovies([]);
      } finally {
  ...
  const scrollByAmount = (direction) => {
    if (scrollRef.current) {
      ...
      let scrollAmount = cardElement
        ? cardElement.offsetWidth + 20
  ...
  if (loading) {
    return (
      <h3 style={{ color: "#aaa", paddingLeft: "20px" }}>{title} 로딩 중...</h3>
    );
  }

  if (movies.length === 0) {
    return (
      <div style={{ color: "#aaa", paddingLeft: "20px" }}>
        {title} 목록을 찾을 수 없습니다.
      </div>
    );
  }
